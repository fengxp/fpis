...
<div class="nav">
	<div class="left">已发布节目</div>
	<div class="right"><a href="<?php echo U('program');?>" class="btn btn_submit mr10">发布</a>&nbsp;&nbsp;</div>
</div>
...
<table width="100%">
	...
	<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr height="30">
			<td><input type="checkbox" name="id" value="<?php echo ($val["pid"]); ?>"/></td>
			<td><?php echo ($val["pro_name"]); ?></td>
			<td><?php echo (getAuditStatus($val["status"])); ?></td>
			<td><?php echo (date('Y-m-d H:i',$val["sub_time"])); ?></td>
			<td>
				<a href="<?php echo U('see',"id=$val[pid]");?>">查看</a> | 
				<a href="javascript:" onclick="return mydelete('<?php echo U('delete',"id=$val[pid]");?>');">删除</a>
			</td>
