...
<table width='100%'>
	<tr>
		<th width="20%">角色名称：</th>
		<td width="80%"><input type="text" name="role_name" class="input" size="40" value="<?php echo ($val["title"]); ?>" /></td>
	</tr>
	<tr>
		<th width="20%">角色权限：</th>
		<td width="80%"></td>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
		...
		<?php if(is_array($data['child'])): $i = 0; $__LIST__ = $data['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><input type="checkbox" name="group[<?php echo ($v["id"]); ?>]" value="<?php echo ($v["id"]); ?>" <?php if(in_array(($v["id"]), is_array($val["rules"])?$val["rules"]:explode(',',$val["rules"]))): ?>checked<?php endif; ?>  /> <?php echo ($v["title"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
		</td>
	</tr>
...
<div class="btn_wrap">
  <div class="btn_wrap_pd">
    <input name="role_id" type="hidden" value="<?php echo ($val["id"]); ?>">
