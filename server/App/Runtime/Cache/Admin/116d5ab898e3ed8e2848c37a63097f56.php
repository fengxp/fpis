...
<table width="100%">
	<thead>
		<tr height="30">
			<td><label class="checkbox"> <input type="checkbox" />ID</label></td>
			<td>管理员名称</td>
			<td>邮箱</td>
			<td>手机号</td>
			<td>登录时间</td>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr height="30">
			...
			<td>
				<a href="<?php echo U('acc_edit',"id=$val[id]");?>">编辑</a> | 
				<a href="javascript:" onclick="return mydelete('<?php echo U('acc_delete',"id=$val[id]");?>');">删除</a>
			</td>
