...
<div class="nav">
	<div class="left">已发布信息</div>
	<div class="right"><a href="<?php echo U('add');?>" class="btn btn_submit mr10">发布</a>&nbsp;&nbsp;</div>
</div>
...
<table width="100%">
	...
	<?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr height="30">
			...
			<td><?php echo (getInfoType($val["type"])); ?></td>
			<td><?php echo (getAuditStatus($val["status"])); ?></td>
			<td><?php echo ($val["length"]); ?></td>
			<td><?php echo (date('Y-m-d H:i',$val["sub_time"])); ?></td>
			<td><?php echo (getUpdateTime($val["update"])); ?></td>
			<td>
				<a href="<?php echo U('see',"id=$val[id]");?>">查看</a> | <a href="<?php echo U('cancle',"id=$val[id]");?>">撤销</a>|
